...
pub const slash_specs = [_]SlashSpec{
    ...
    .{ .kind = .login, .command = "/login", .help_entry = "/login", .completion_description = "sign in with Vercel, Grok, or Codex", .presentation_category = .account },
    .{ .kind = .logout, .command = "/logout", .help_entry = "/logout [vercel|grok|codex]", .completion_description = "sign out of a provider session", .presentation_category = .account, .has_args = true, .accepts_payload = true },
    ...
    .{ .kind = .credits, .command = "/credits", .aliases = &.{ "/balance", "/xai-usage" }, .help_entry = "/credits (/balance, /xai-usage)", .completion_description = "show gateway credits or SuperGrok usage", .presentation_category = .account, .requires_prompt_credential = true },
